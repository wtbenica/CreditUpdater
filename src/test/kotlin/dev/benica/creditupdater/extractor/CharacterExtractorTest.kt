    ...
    fun shouldExtractCharactersAndTeamAndInsertThemAndTheirAppearancesIntoTheDatabase() {
        ...
        getDbConnection().use { conn ->
            conn.createStatement().use {
                ...
                assertEquals("", rs.getString("details"))
                assertNull(rs.getString("notes"))
                assertEquals("Superman [Clark Kent]; Green Lantern [Hal Jordan]; The Flash [Barry Allen]; Aquaman [Arthur Curry]; Martian Manhunter [J'onn J'onzz]", rs.getString("membership"))
                assertFalse(rs.next())
