...
class CharacterRepositoryTest {
    ...
    fun shouldThrowExceptionIfDatabaseThrowsException() {
        ...
        whenever(queryExecutorMock.executePreparedStatementBatch(any(), any(), any(), any())).thenAnswer {
            throw SQLException()
        }
    ...
    fun shouldThrowAnErrorAndLogErrorMessageWhenInsertCharacterAppearancesFails() {
        ...
        whenever(
            queryExecutorMock.executePreparedStatementBatch(any(), any(), any(), any())
    ...
    fun shouldThrowSQLExceptionWhenInsertCharacterFails() {
        ...
        whenever(
            queryExecutorMock.executePreparedStatementBatch(
                sql = any(),
                autoGeneratedKeys = any(),
                conn = anyOrNull(),
                batchAction = any()
            )
        ).thenAnswer {
            val action = it.getArgument<(PreparedStatement) -> Unit>(3)
